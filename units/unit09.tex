						...
						\begin{enumerate}
							\item \question{$[(\lnot q) \rightarrow (\lnot p)] \rightarrow (p \rightarrow q)$}
								\begin{center}
									\begin{tabular}{| c c | c c | c | c | c |}
										\hline
										\tablehead{$p$} & \tablehead{$q$} & \tablehead{$\lnot p$} & \tablehead{$\lnot q$} & \tablehead{$(\lnot q) \rightarrow (\lnot p)$} & \tablehead{$p \rightarrow q$} & \tablehead{$\bigl[(\lnot q) \rightarrow (\lnot p)\bigr] \rightarrow (p \rightarrow q)$}\\
										\hline
										T & T & F & F & T & T & T\\
										T & F & F & T & F & F & T\\
										F & T & T & F & T & T & T\\
				...
				\begin{enumerate}
					...
					\item \question{Suppose we want to define a new connective, the \emph{exclusive disjunction}, also called the \emph{``exclusive or''}, which will be written $+$. By $p + q$, we denote ``$p$ or $q$, but not both''. Construct a truth table for this connective.}
						\begin{center}
							\begin{tabular}{|c c | c|}
								\hline
								\tablehead{$p$} & \tablehead{$q$} & \tablehead{$p + q$}\\
								\hline
								T & T & F\\
								T & F & T\\
								F & T & T\\
					...
					\item \question{Use the law of double negation and De Morgan's laws to rewrite the following statements so that the not symbol ($\lnot$) does not appear outside parentheses.}
						\begin{enumerate}[label=(\alph*)]
							...
							\item \question{$\lnot \Bigl[\bigl(p \lor (p \rightarrow q)\bigr) \lor (p \land q)\Bigr]$}
								...
								\end{align*}
						\end{enumerate}
					\item \question{Determine whether or not the following statements are equivalent:\\ $\lnot p \land (p \land \lnot q)$ and $\lnot \bigl(p \lor (p \rightarrow q)\bigr)$}
						\begin{align*}
